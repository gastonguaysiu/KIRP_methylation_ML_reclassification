...
# Load your data
data <- read.csv("rpm_RNA_seq2.csv", header = TRUE)
leftrow <- as.data.frame(data[,1])
data <- data[,-1]

filenames <- read.csv("filenames_all.csv")
...
notes <- merge(x = ref, y = new_clusters, by.x = "miRNA_filename", by.y = "sample") %>%
  select(-c(5,6,15:28))

notes2 <- anti_join(notes, filename_trainning, by = "miRNA_filename")
